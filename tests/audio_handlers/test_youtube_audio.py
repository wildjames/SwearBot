...
def test_get_video_id_various_urls(tmp_cache_root):
    ...
    assert (
        mod._get_video_id("https://www.youtube.com/embed/ZYXWVUTSRQP") == "ZYXWVUTSRQP"
    )
...
async def test_fetch_audio_pcm_success(tmp_cache_root, monkeypatch):
    ...
    monkeypatch.setattr(
        mod, "get_youtube_track_metadata", fake_get_youtube_track_metadata
    )
    ...
    expected = (
        cache_dir
        / f"SOMEID12345_{mod.DEFAULT_SAMPLE_RATE}Hz_{mod.DEFAULT_CHANNELS}ch.pcm"
    )
...
async def test_fetch_audio_pcm_auth_error(tmp_cache_root):
    ...
    with pytest.raises(NotImplementedError):
        await mod.fetch_audio_pcm(
            "http://example.com", username="user", password="pass"
        )
...
@pytest.mark.asyncio
async def test_get_youtube_track_metadata_success_and_format(
    tmp_cache_root, monkeypatch
):
    mod = import_module(tmp_cache_root)
...
async def test_search_youtube_success(tmp_cache_root, monkeypatch):
    ...
    assert results[0] == (
        "https://www.youtube.com/watch?v=VIDA1234567",
        "First Video",
        120,
    )
    # Second valid entry: skip invalids, next is VIDD
    assert results[1] == (
        "https://www.youtube.com/watch?v=VIDD4567890",
        "Third Video",
        200,
    )
